...
token = '6678218319:AAHOkjxAQ8yCm-GqiYEiyaUWPOYfqDSL9yY'
...
def menu(message):
    ...
    markup.add(types.InlineKeyboardButton('Программа лояльности🎁', callback_data='program'))
    markup.add(types.InlineKeyboardButton('👍Отзывы', url='https://t.me/+H5RiFALzIPw2Y2Qy'))
    markup.add(types.InlineKeyboardButton('Оформить заказ👤',url='https://t.me/LiteScore'))
    p1 = open('media/dp3.jpg','rb')
    bot.send_photo(message.chat.id,photo=p1  ,caption=f'<b>Привет, {message.from_user.first_name}!</b>\n\nДанный бот создан, чтобы устроить более качественное взаимодействие между вами и Poizon.\n\nЕсли вы готовы оформить заказ и уже знаете как и что у нас происходит - можете сразу писать сюда👉@LiteScore\n\nЧто будем делать?',
...
def handle_how_to_use(call):
    ...
    p1 = open('media/how_to_use.jpg', 'rb')
...
def handle_shoes(call):
    chat_id = call.message.chat.id
    markup = types.InlineKeyboardMarkup()
    p12 = open('media/secondbuttons.jpg', 'rb')
...
def handle_program(call):
    ...
    bot.send_message(call.message.chat.id, '<b>🎁 Комиссии и система лояльности</b>\n\n<b>При этом, курс юаня у нас один из самых выгодных на рынке.</b>\n\nТакже, совсем недавно, мы ввели систему лояльности, которая может снизить комиссию указанную выше — вплоть до <b>-25%</b>\n\n📌<b> Работает это при повторном заказе, либо от 2-ух вещей в одном заказе:</b>\n\n✅От 2-х вещей - скидка <b>10%</b> на комиссию\n✅От 3-х вещей - скидка <b>15%</b> на комиссию\n✅От 4-х вещей - скидка <b>20%</b> на комиссию\n✅От 5-и вещей - скидка <b>25%</b> на комиссию\n\nРовно так же работает и при <b>повторных заказах</b>. Если Вы заказываете по одной вещи, но уже третий раз, то скидка на комиссию — будет <b>15%</b>.\n\nПри достижении 5-и заказов, либо 5 вещей в одном заказе — скидка <b>25% фиксируется за Вами навсегда</b> и будет действовать при любом заказе.\n\nА самое интересное касается тех, кто оформляет <b>6 и более вещей в одном заказе</b>, так как одна из них будет идти <b>БЕЗ комиссии.</b>Чтобы всё было честно, определяем мы такую вешь с помощью «🎲», просто бросив кубик прямо в чате с @LiteScore\n\n<b>Нафармить хорошую скидку на комиссию вы можете уже сейчас.</b>', parse_mode='html',reply_markup=markup)
...
def handle_what(call):
    ...
    markup.add(button1)
    markup.add(button2)
    p7 = open('media/what_of_poizon.jpg','rb')
    p14 = open('media/16prover.png','rb')
...
def handle_qr_code(call):
    ...
    button1 = types.InlineKeyboardButton('Назад↩️', callback_data='back_to_how_to_use')
    markup.add(button1)
    p4 = open('media/rolegit.png', 'rb')
    p13 = open('media/qr3.png','rb')
...
def handle_why_of_us(call):
    ...
    markup.add(button1)
    p9 = open('media/why_of_us.png','rb')
...
def handle_pay_upon_receipt(call):
    ...
    button1 = types.InlineKeyboardButton('Назад↩️', callback_data='back_to_question')
    markup.add(button1)
    p10 = open('media/pay_upon_receipt.jpg','rb')
...
def handle_many_things(call):
    ...
    button1 = types.InlineKeyboardButton('Назад↩️', callback_data='back_to_question')
    markup.add(button1)
    p12 = open('media/many_things.jpg', 'rb')
